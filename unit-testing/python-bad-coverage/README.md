...
This project demonstrates what happens when a Python application lacks adequate test coverage. You'll see how Poetry, pytest, Behave, Coverage.py, Pylint, and Jenkins work together in a CI/CD pipeline, but more importantly, you'll observe the consequences of incomplete testing.
...
cd ser516public/unit-testing/python-bad-coverage
```
...
**PROBLEM IDENTIFIED:** Only minimal pytest tests exist.
...
- Only **2 test methods** exist in `test_calculator.py` (test_add, test_subtract)
- Most Calculator methods are **NOT tested** (multiply, divide, power, square_root)
- Exception handling is **NOT tested**

**Expected output:** Test results showing `Tests run: 2, Passed: 2` with LOW coverage warning

**What's Missing:**
- No tests for multiply and divide operations
- No edge case testing (divide by zero, negative square root)
- No tests for power and square_root methods
- See missing tests in [test_calculator.py](tests/test_calculator.py)

### Step 4: Run BDD Tests with Behave

**PROBLEM IDENTIFIED:** Complete Behave test scenarios defined, but unit tests don't match.
...
- However, **unit tests are insufficient** - only 2 pytest unit tests exist
...
- **Low line coverage** (~30-40%) for the Calculator class
- **Missing branch coverage** for conditional logic
- Large **RED sections** indicating untested code paths

**What You'll See:**
- multiply, divide, power, and square_root methods mostly untested (RED)
- Exception handling paths not executed (RED)
- Only add and subtract methods are covered (GREEN)
- Coverage percentage **BELOW acceptable standards** (typically need 80%+)
- Note: `pyproject.toml` sets `--cov-fail-under=30` (intentionally LOW threshold)
